...
__forceinline bool CheckRenderIsCompatible()
{
	...
	{
		MessageBoxA(nullptr, "You are running with graphics options that BearPaste does not support.\n"
	...
	if (iLevel < 90)
	{
		std::string fmt = std::format("You are running with graphics options that BearPaste does not support.\nPlease remove -dxlevel {} from your launch options and reinject.\0", iLevel);
...
void CCore::Load()
{
	...
	SDK::Output("BearPaste", "Loaded", { 175, 150, 255, 255 });
}
...
void CCore::Unload()
{
	...
	SDK::Output("BearPaste", "Unloaded", { 175, 150, 255, 255 });
}
